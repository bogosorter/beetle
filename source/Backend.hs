...
compileExpression env expression = case expression of
    ...
    Match scrutinee branches defaultBranch t -> do
        ...
        let encloseBranch :: Label -> (Int, String, Closures.Type, Expression) -> State CompilationState (Operand, Label)
            encloseBranch label (_, name, t, body) = do
                ...
                bodyLabel <- getBasicBlock
                putStatement $ Jump mergeLabel
                putStatement $ EmptyLine

                return (bodyRegister, bodyLabel)

        branchOperandsAndLabels <- mapM (uncurry encloseBranch) (zip labels branches)
        (defaultOperand, defaultLabel) <- case defaultBranch of
            Just branch -> do
                putLabel defaultLabel
                defaultOperand <- compileExpression env branch
                defaultLabel <- getBasicBlock
                putStatement $ Jump mergeLabel
                putStatement $ EmptyLine
                return (defaultOperand, defaultLabel)
            Nothing -> do
                putStatement $ Comment "The default label is required but never reached"
                putLabel defaultLabel
                putStatement $ Jump mergeLabel
                putStatement $ EmptyLine
                return (poison, defaultLabel)

        putLabel mergeLabel
        let phiSources = branchOperandsAndLabels ++ [(defaultOperand, defaultLabel)]
        resultRegister <- reserveRegister
